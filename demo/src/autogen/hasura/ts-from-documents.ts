...
export async function fetchVehicleGraphFieldsById({
  ...
  const query = await apolloClient.query<FetchVehicleGraphFieldsByIdQuery>({ query: FetchVehicleGraphFieldsByIdDocument, variables: { id: vehicleId }, ...options });
...
export async function insertVehicleGraphFieldsObject({
  ...
  return {
    ...mutation,
    vehicleGraphFields: mutation && mutation.data && mutation.data.insert_vehicle && mutation.data.insert_vehicle!.returning && mutation.data.insert_vehicle!.returning[0]
  };
}
...
export async function updateVehicleGraphFieldsById({
  ...
  const mutation = await apolloClient.mutate<UpdateVehicleGraphFieldsByIdMutation, UpdateVehicleGraphFieldsByIdMutationVariables>({
    mutation: UpdateVehicleGraphFieldsByIdDocument,
  ...
  return {
    ...mutation,
    vehicleGraphFields: mutation && mutation.data && mutation.data.update_vehicle && mutation.data.update_vehicle!.returning && mutation.data.update_vehicle!.returning[0]
  };
}
...
export async function updateVehicleGraphFieldsObjects({
  ...
  const mutation = await apolloClient.mutate<UpdateVehicleGraphFieldsMutation, UpdateVehicleGraphFieldsMutationVariables>({
    mutation: UpdateVehicleGraphFieldsDocument,
    ...options
  });
...
export async function removeVehicleModelById({
  ...
  const mutation = await apolloClient.mutate<RemoveVehicleModelByIdMutation, RemoveVehicleModelByIdMutationVariables>({
